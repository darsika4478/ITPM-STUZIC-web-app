 ...
 *   currentPhoto – URL of current avatar (or '' if none)
 *   onSave      – (blob: Blob) => Promise<void>  — called with the cropped image blob
 *   onDelete    – () => Promise<void>             — called to remove current photo
 *   onClose     – () => void                      — close without saving
 */
const AvatarEditorModal = ({ open, currentPhoto, onSave, onDelete, onClose }) => {
    const fileInputRef = useRef(null);

    // The image loaded into the editor (always a data-URL for CORS safety)
    ...
    // Track whether user loaded a new file (vs. editing existing photo)
    const [hasNewFile, setHasNewFile] = useState(false);

    // ── Convert external URLs (Firebase Storage) to data-URLs ──
    // This avoids CORS / tainted-canvas errors when cropping.
    useEffect(() => {
        if (!open) return;
        if (!currentPhoto) {
            setImageSrc('');
        ...
        fetch(currentPhoto)
            ...
            .catch((err) => {
                ...
                if (!cancelled) {
                    setImageSrc('');
                    setError('Could not load current photo.');
                    setLoading(false);
                }
            });
        return () => {
    ...
    // Save: crop the image and pass the blob up
